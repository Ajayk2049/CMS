...
export default function AdminPortal() {
  ...
  const getTabBadgeCount = (tabId) => {
    if (tabId === 'requests') {
      ...
      const pendingCampaignsCount = campaigns.filter(c => c.paymentStatus === 'completed' && c.approvalStatus === 'pending').length;
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden bg-background">
        ...
        <main className="flex-1 overflow-y-auto p-6">
          {notification.message && (
            <div
              className={`mb-6 p-4 rounded-2xl border text-xs font-semibold flex justify-between items-center animate-fade-in shadow-md ${notification.type === 'error'
            ...
            >
              ...
              <button
                ...
                className="p-1 hover:bg-white/20 rounded-lg transition-colors cursor-pointer text-white/80 hover:text-white shrink-0"
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  >
                    ...
                    <p className="text-[10px] text-muted-foreground mt-1 font-semibold">
                      <span className="text-[#0069a8] font-bold">{campaigns.filter(c => c.approvalStatus === 'pending').length} pending review</span> / {campaigns.length} total
                    </p>
                  </div>
                              ...
                              <tr key={rate._id} className="hover:bg-card/20 transition-colors duration-200">
                                <td className="p-4 pl-6 font-bold text-foreground">
                                  <span className={`inline-flex items-center gap-1 text-[9px] font-extrabold uppercase px-2 py-0.5 rounded ${rate.mediaType === 'image' ? 'bg-purple-500/10 text-purple-500 border border-purple-500/20' : 'bg-blue-500/10 text-blue-500 border border-blue-500/20'}`}>
                                    ...
                                    <span>{rate.mediaType === 'image' ? 'Static Image' : 'Dynamic Video'}</span>
                                  </span>
                                </td>
